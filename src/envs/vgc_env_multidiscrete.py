"""
VGC Environment with MultiDiscrete action space for RL training.

This environment uses MultiDiscrete([107, 107]) action space where each
position independently selects from 107 actions.

Action encoding per position:
- 0: pass
- 1-6: switch to team member 1-6
- 7+: moves with targets and gimmicks (formula: 7 + move_idx*5 + target_offset + gimmick*20)

Usage:
    from src.envs import VGCEnvMultiDiscrete
    from sb3_contrib import MaskablePPO

    env = VGCEnvMultiDiscrete()
...
from ..utils.action_masking import ACTION_SPACE_SIZE, get_valid_actions_per_position
...
class VGCEnvMultiDiscrete(Env):
    """
    VGC Environment with MultiDiscrete([107, 107]) action space.

    Each active Pokemon independently selects from 107 possible actions.
    This is simpler than the joint Discrete(11449) space but doesn't
    enforce joint-legality constraints at the action level.
    ...
    def __init__(
        ...
        # MultiDiscrete action space: [107, 107] for two active Pokemon
    ...
    def action_masks(self) -> npt.NDArray[np.bool_]:
        """
        Get the action mask for MultiDiscrete action space.

        Returns a concatenated mask of shape (214,) = (107 + 107).
        For MaskablePPO with MultiDiscrete, masks are concatenated.
        """
    ...
    def step(
        ...
        Args:
            action: Array of shape (2,) with actions for each active Pokemon.
