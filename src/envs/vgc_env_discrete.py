...
class VGCEnvDiscrete(Env):
    """
    VGC Environment with Discrete(11449) action space for RL training.

    This environment uses a flattened joint-action space where each action
    index represents a (action_pos0, action_pos1) pair. This properly
    enforces joint-legality constraints via action masking.

    Action space: Discrete(11449) = 107*107
    - Flat index maps to (a0, a1) via: a0 = idx // 107, a1 = idx % 107

    Use with MaskablePPO from sb3-contrib for best results.
    ...
    def __init__(
        ...
        # Each action index maps to (action_pos0, action_pos1) via flat_to_action_pair()
    ...
    def action_masks(self) -> npt.NDArray[np.bool_]:
        ...
        For MaskablePPO with Discrete(11449), this returns a flattened
        joint-action mask of shape (11449,) where each index corresponds
        to a (action_pos0, action_pos1) pair.

        This properly handles joint-legality constraints (e.g., both can't tera,
        both can't switch to same Pokemon) by only marking valid pairs as True.
    ...
    def step(
        ...
        Args:
            action: Flat action index (0 to 11448) representing joint action for both Pokemon.
                    Converted to (action_pos0, action_pos1) via flat_to_action_pair().
        ...
        # Send action pair to the player
