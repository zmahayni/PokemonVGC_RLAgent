...
class Singles_Env_v1(SinglesEnv):
    def __init__(self, battle_format, team=None):
        ...
        self.observation_spaces = {
            agent: Box(low=0.0, high=1.0, shape=(14,), dtype=np.float32)
    ...
    def embed_battle(self, battle):
        ...
        if len(battle.available_moves) == 1 and battle.available_moves[0].id in [
            "struggle",
            "recharge",
        ]:
        ...
        for i, m in enumerate(moves):
            type_effectiveness = min(
                battle.opponent_active_pokemon.damage_multiplier(m.type), 4.0
            )
            type_effectiveness_norm = type_effectiveness / 4.0
